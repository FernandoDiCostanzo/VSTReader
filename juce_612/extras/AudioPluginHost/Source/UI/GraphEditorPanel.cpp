    ...
    PluginComponent (GraphEditorPanel& p, AudioProcessorGraph::NodeID id)  : panel (p), graph (p.graph), pluginID (id)
    {
        ...
        if (auto f = graph.graph.getNodeForId (pluginID))
        {
            if (auto* processor = f->getProcessor())
            {
                ...
                GraphDocumentComponent* gdc = dynamic_cast<GraphDocumentComponent*>(p.getParentComponent());
                if (gdc) {
                    gdc->papcw->setProcessor(processor);
                }
                for (auto* param : processor->getParameters()) {
                    if (param->isAutomatable()) {
                        DBG("[Total params size : " + std::to_string(processor->getNumParameters()) + "] | " + "[Automatable Params] : " + param->getName(1000).toStdString() + " ");
                    }
                }
...
void GraphDocumentComponent::init()
{
    ...
    papcw.reset(new PluginAutParamsCompWrapper());
    addAndMakeVisible(*papcw);
...
void GraphDocumentComponent::resized()
{
    ...
    papcw->setBounds(r.removeFromBottom(150));
