...
public:
         ...
         node (n), type (t)
    {
        ...
        auto* ui = createProcessorEditor(*node->getProcessor(), type);
        if (ui) {
       ...
       #else
        setTopLeftPosition (node->properties.getWithDefault (getLastXProp (type),100),
                            node->properties.getWithDefault (getLastYProp (type), 100));
       #endif

        node->properties.set (getOpenProp (type), true);

        int w = ui ? ui->getWidth() : 400;
        int h = ui ? ui->getHeight()  : 400;
        
        ui->setTopLeftPosition({ 20,20 });
        setVisible (true);
    }
